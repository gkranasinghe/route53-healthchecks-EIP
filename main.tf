...
resource "aws_sns_topic_subscription" "this" {
  ...
  protocol               = var.subscription_endpoint_protocol #(Required) Protocol to use. Valid values are: sqs, sms, lambda, firehose, and application. Protocols email, email-json, http and https are also valid but partially supported. See details below.
...
resource "aws_route53_health_check" "this" {
  ...
  port              = "22"
  type              = "TCP"   #type - (Required) The protocol to use when performing health checks. Valid values are HTTP, HTTPS, HTTP_STR_MATCH, HTTPS_STR_MATCH, TCP, CALCULATED, CLOUDWATCH_METRIC and RECOVERY_CONTROL.
...
resource "aws_cloudwatch_metric_alarm" "this" {
  ...
  alarm_description         = "This metric monitors ${var.name} SFTP service endpoint ( ${var.ip_address} ) whether it is UP or Down"
